        ...
        if (url[i] != undefined && url[i] != null) {
          if (url[i].id.split("_")[0] == "newaudio") {
            let parent_id = document
              .getElementById(url[i].id)
              .closest("tr")
              .id.split("-")[1];
            let fileInput;

            const audio_file = document.querySelectorAll('input[type="file"]');

            for (let j = 0; j < audio_file.length; j++) {
              if (
                audio_file[j].id ==
                "id_medias-" + parent_id + "-path_to_file"
              ) {
                fileInput = audio_file[j];
              }
            }

            let file = await fetch(url[i].src)
              .then((r) => r.blob())
              .then(
                (blobFile) =>
                  new File(
                    [blobFile],
                    "user_recording_" + parent_id + "_.mp3",
                    {
                      type: "mp3",
                    }
                  )
  ...
  navigator.mediaDevices
    ...
    .then((mediaStream) => {
      ...
      mediaRecorder.onstop = () => {
        ...
        _parent.append(recordedMedia, downloadButton);
